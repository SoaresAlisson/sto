...
```
## f 
usar r"()" em f para utilizar menos caracteres
```{r}
f  <- function(x){
x |> r"()"
}
...
## stopwords faltantes
```{r}
sw <- stopwords::stopwords(language = "en", source = "snowball", simplify = TRUE)

sw[!sw %in% show_sw("en", as_vector = TRUE)]
```
```{r}
sw <- stopwords::stopwords(language = "pt", source = "snowball", simplify = TRUE)

sw[!sw %in% show_sw("pt", as_vector = TRUE)]
```
...
diff_string <- function(string1, string2) {
  Reduce(setdiff, strsplit(c(string1, string2), split = ""))
  # setdiff(strsplit(c(string1, string2), split = ""))
}

diff_string("blabla", "blable")
diff_string("blabla", "bla ble") 

List <- strsplit(c(string1, string2), split = "")
List[[1]](List[[1]] == List[[2]])

Filter(fun (List[[1]] == List[[2]) ]
)
