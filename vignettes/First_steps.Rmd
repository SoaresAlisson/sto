...
sto::ll("sto dplyr")
```
It is possible to print the vector of  `library(package)` using the option `print=TRUE`
```{r ll print}
...
```
Another function available is `nothing()`, that does nothing, but it is useful when you want to use the native R pipe, so you can comment and uncomment the lines of the pipe, without having to change the last pipe.
